...
void DlgApplication::OnStart()
{
    ...
    FILE *stream = fopen("dlog/env_airportExterior.dlog", "rb");
    if (stream == NULL)
    {
        printf("Error: unable to open dlog file\n");
    }
    readMetaStream(stream, &(this->header));
    DlgRead(stream, &(this->dlg), 0);
    fclose(stream);
...
DlgRead
    ...
    */
    ...
    for (uint32_t i = 1; i < *(uint64_t *)(dlg.children[14]->children[0]->data.staticBuffer); ++i)
    {
        ...
        sprintf(textBuffer, "##%s%d", metaClassName, header_id);
        ...
        if (firstframe)
        {
            ed::SetNodePosition(header_id, ImVec2(420, 20 + i * 100));
