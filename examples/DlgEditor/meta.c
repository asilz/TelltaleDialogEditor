...
{
    enum Type type = searchDatabase("database/protonDatabase.db", node->typeSymbol);
    RenderFunction renderFunction = metaClassDescriptions[type].render;
    if (renderFunction == NULL)
    {
        printf("Error: Render function not implemented for %s, type = %lx\n = %d", metaClassDescriptions[type].name, node->typeSymbol, type);
        exit(-1);
        return -1;
    }
    return renderFunction(node, flags);
}

char *getMetaClassName(uint64_t typeSymbol)
{
    return metaClassDescriptions[searchDatabase("database/protonDatabase.db", typeSymbol)].name;
}
